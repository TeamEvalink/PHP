...
function getActiveByParam(page,page_totle,per_page){
	...
	$.post("/sysacl/getActiveByParam/"+page+'/'+page_totle+'/'+per_page,
	...
	//Ajax
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        window.location.href="/sysacl"; 
                    }
                });
      //Ajax end
}
function updateuseracl(){
	$.post("/sysacl/Act_update", $("#useracledit").serialize(),
			function (data){
				if (data.status==0) {
				alert(data.info);
				return ;
...
function adduseracl(){
	...
	$.post("/sysacl/addActive", $("#useracladd").serialize(),function(data){
			if (data.status==0) {
				alert(data.info);
				return ;
			}
...
function act_update(adId){
	location.href="/sysacl/update/"+adId;
}
function checkAll(o){
	if( o.checked == true ){
